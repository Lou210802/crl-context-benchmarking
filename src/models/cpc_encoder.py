...
class CPCContextEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.feature_dim = int(state_dim + action_dim + 1)
        self.latent_dim = int(latent_dim)
        self.hidden_dim = int(hidden_dim)
        self.predict_horizon = max(1, int(predict_horizon))
        self.temperature = float(temperature)

        # 1. Recurrent GRU Backbone
        self.gru = nn.GRU(
        ...
        # Auxiliary Dynamics MSE Decoder Head
    ...
    def forward(self, history_sequence: torch.Tensor) -> torch.Tensor:
        """Forward pass returning latent context z_t."""
    ...
    def compute_loss(
        ...
        """
        Computes Multi-Step InfoNCE contrastive predictive loss with SupCon & Aux Dynamics MSE.
        """
        history_sequence = history_sequence.to(self.device)
        batch_size = history_sequence.size(0)

        z_t = self.encode(history_sequence)
        z_t_norm = F.normalize(z_t, p=2, dim=-1)

        # 1. Supervised Context Contrastive (SupCon) Loss
        ...
        # False-Negative Same Context Masking for InfoNCE
        ...
        # 2. Multi-Step InfoNCE & Dynamics MSE Horizon Loop
        for k in range(eval_horizon):
            target_next_s_k = target_next_state[:, k, :]
            target_r_k = target_reward[:, k, :]
            curr_s_k = state[:, k, :] if (state.ndim > 2 and state.shape[1] > k) else state
            ...
            pred_in_k = z_t_norm
            pred_k = self.predictors[k](pred_in_k)
            ...
            act_k = action[:, k, :] if (action.ndim > 2 and action.shape[1] > k) else action
        ...
        # Balanced Loss Assembly Weights
        lambda_infonce = 1.0
        lambda_supcon = 0.1
        lambda_aux = 1.0

        combined_loss = (
            lambda_infonce * mean_infonce_loss
            + lambda_supcon * supcon_loss
            + lambda_aux * mean_aux_loss
        )
